...
def relationship_status(from_member, to_member, social_graph):
    ...
    if to_member in social_graph[from_member]["follow"]:
        if from_member in social_graph[to_member]["follow"]:
            return("friends")
        elif from_member not in social_graph[to_member]["follow"]:
            return("follower")
    elif from_member in social_graph[to_member]["follow"]:
        return("followed by") 
...
def eta(first_stop, second_stop, route_map):
    ...
    if second_stop == first_stop:
        ...
        if direction in route_map:
            ...
            midpoint = new_list.index(first_stop) + 1

            if midpoint > 2:
                midpoint -= len(keys)

            #Routes
            route1 = (new_list[midpoint],second_stop)
            
            route2 = (first_stop,new_list[midpoint])
            
            return((route_map[route2]["travel_time_mins"]) + route_map[route1]["travel_time_mins"])
